...
<main>
  <div class="options">
    ...
    <button on:click={onlyFavSongs}>Solo canzoni preferite</button>
  </div>
  <br>
...
<style>
  ...
  .options {
    width: 100vw; /* Imposta la larghezza al 100% della larghezza della finestra visualizzata */
    position: fixed; /* Per posizionare l'elemento rispetto alla finestra visualizzata */
    display: flex;
    flex-direction: row;
    justify-content: space-around; /* Per distribuire uniformemente gli elementi lungo il contenitore */
    align-items: center; /* Per centrare verticalmente gli elementi */
    bottom: 0; /* Fissa l'elemento al fondo della finestra */
    left: 0; /* Posiziona l'elemento all'estrema sinistra della finestra */
    padding: 10px; /* Aggiunge un padding per spaziatura */
  }
</style>